...
export function fetchGamesList(): ThunkAction<Promise<void>, {}, {}, AnyAction> {
    return async (dispatch: ThunkDispatch<{}, {}, AnyAction>) => {
        try {
            ...
            console.error(err);
            dispatch({type: Actions.UPDATE_GAMES_LIST_FAILURE, payload: "Failed to fetch games - " + err});
        }
    }
}
...
export function joinGameServer(gameId: number): ThunkAction<Promise<JoinInfo>, {}, {}, AnyAction> {
    return async (dispatch: ThunkDispatch<{}, {}, AnyAction>) => {
        ...
        console.debug(r);
        dispatch({type: Actions.JOIN_GAME_SERVER, payload: {Id: gameId, JoinInfo: r}});
        return r;
    }
}
...
export function establishGameConnection(gameId: number): ThunkAction<Promise<HubConnection>, RootState, {}, AnyAction> {
    return async (dispatch: ThunkDispatch<RootState, {}, AnyAction>, getState) => {
        ...
        hubConnection.onclose((err?: Error) => {
            ...
            dispatch({type: Actions.CONNECTION_ERROR, payload: "Connection closed "+err});
            navigate("GamesList", {})
        });

        await hubConnection.start();
        console.debug(`Connected to game server name: ${game.Name}, id: ${game.Id}, url: ${game.Server.Url}`);
        dispatch({type: Actions.ESTABLISH_GAME_SERVER_CONNECTION, payload: {Id: game.Id, HubConnection: hubConnection}});
...
export function enterGame(gameId: number): ThunkAction<Promise<void>, RootState, {}, AnyAction> {
    return async (dispatch: ThunkDispatch<RootState, {}, AnyAction>, getState) => {
        ...
        dispatch({type: Actions.ENTER_GAME, payload: {Id: game.Id, GameInfo: r2.content}})
    }
}
...
export function joinEstablishAndEnterGame(gameId: number): ThunkAction<Promise<void>, RootState, {}, AnyAction> {
    return async (dispatch: ThunkDispatch<RootState, {}, AnyAction>) => {
        dispatch({type: Actions.CONNECTION_ERROR, payload: {Id: gameId, error: undefined}});
        try {
            ...
            console.error(err);
            dispatch({type: Actions.CONNECTION_ERROR, payload: {Id: gameId, error: `Failed to connect to server ${gameId} - ${err}`}});
            navigate("GamesList", {})
        }
    }
}
...
export function fetchNotifications(gameId: number): ThunkAction<Promise<void>, RootState, {}, AnyAction> {
    return async (dispatch: ThunkDispatch<RootState, {}, AnyAction>, getState) => {
        const game: Game = getState().gamesList.Games[gameId.toString()];
        dispatch({type: Actions.UPDATE_NOTIFICATIONS_ERROR, payload: {Id: gameId, error: undefined}});
        try {
            const r = await InvokeGetNotifications(game.HubConnection!!) as any;
            dispatch({type: Actions.UPDATE_NOTIFICATIONS, payload: {Id: game.Id, Notifications: r.content}});
        }
        catch(err) {
            console.error(err);
            dispatch({type: Actions.UPDATE_NOTIFICATIONS_ERROR, payload: {Id: game.Id, error: "Failed to update notifications - "+err}});
        }
    }
}
...
export function markNotificationsAsRead(gameId: number, notificationIds: number[]): ThunkAction<Promise<void>, RootState, {}, AnyAction> {
    return async (dispatch: ThunkDispatch<RootState, {}, AnyAction>, getState) => {
        ...
        try {
            ...
            dispatch({type: Actions.UPDATE_NOTIFICATIONS, payload: {Id: game.Id, Notifications: Notifications}});
        }
        catch(err) {
            console.error(err);
...
export function setNotificationTypeEnabled(gameId: number, notificationType: string, enabled: boolean): ThunkAction<Promise<void>, {}, {}, AnyAction> {
    return async (dispatch: ThunkDispatch<{}, {}, AnyAction>) => {
        dispatch({type: Actions.SET_NOTIFICATION_TYPE_ENABLED, payload: {Id: gameId, type: notificationType, enabled: enabled}});
    }
}

export function setNotificationsEnabled(gameId: number, enabled: boolean): ThunkAction<Promise<void>, {}, {}, AnyAction> {
    return async (dispatch: ThunkDispatch<{}, {}, AnyAction>) => {
        dispatch({type: Actions.SET_NOTIFICATIONS_ENABLED, payload: {Id: gameId, enabled: enabled}});
    }
}
...
export function fetchStations(gameId: number): ThunkAction<Promise<void>, RootState, {}, AnyAction> {
    return async (dispatch: ThunkDispatch<RootState, {}, AnyAction>, getState) => {
        const game: Game = getState().gamesList.Games[gameId.toString()];
        dispatch({type: Actions.UPDATE_STATIONS_ERROR, payload: {Id: gameId, error: undefined}});
        try {
            ...
            dispatch({type: Actions.UPDATE_STATIONS, payload: {Id: game.Id, Stations: stations}});
        }
        catch(err) {
            console.error(err);
            dispatch({type: Actions.UPDATE_STATIONS_ERROR, payload: {Id: game.Id, error: "Failed to update stations - "+err}});
        }
    }
}
