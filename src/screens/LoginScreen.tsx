...
class LoginScreen extends React.Component<Props, State> {
    ...
    handleLoginPress = async () => {
        ...
        const success = await this.props.login(this.state.user, this.state.password);
        if (success)
        ...
        return (
            <SafeAreaView style={styles.container}>
                ...
                <View style={styles.form}>
                    ...
                    <Text style={{color: this.props.theme.colors.error}}>{this.props.login_error}</Text>}

                    {this.props.auth_status === AUTH_STATUS.SUCCESS &&
                    <Text style={{color: this.props.theme.colors.accent}}>Logged in
                        as {this.props.auth?.userName}</Text>}

                    <Button style={{marginTop: 50}} onPress={this.handleLoginPress} mode="contained"
                            loading={this.props.auth_status === AUTH_STATUS.LOADING}>
                        Login
                    </Button>
                </View>
            </SafeAreaView>
